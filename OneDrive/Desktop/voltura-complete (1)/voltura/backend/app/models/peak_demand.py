...
class PeakDemand(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    __tablename__ = "peak_demand"
    __table_args__ = (
        Index("ix_peak_demand_feeder_date", "feeder_id", "date"),
    )
    ...
    demand_mw: Mapped[float] = mapped_column(Numeric(12, 3), nullable=False)

    feeder: Mapped["Feeder"] = relationship(lazy="joined")

    def __repr__(self) -> str:
        return f"<PeakDemand {self.date} {self.demand_mw}MW>"
